...
SUPPORTED_EXTENSIONS = {".mp3", ".aac", ".m4a", ".wav", ".flac", ".ogg", ".opus"}
...
class CastPlayer:
    ...
    def play_random(self):
        ...
        if len(self.three_last_played_files) > 2:
            self.three_last_played_files.pop()
...
class ChromecastPlayerUI:
    ...
    def __init__(self, player: CastPlayer, total_tracks: int, music_root):
        ...
        self.play_next = (
            True  # If player is idle and finished, play next random track if true
        )
        self.HELP_LINES = [
    ...
    def _draw(self, stdscr, max_y, max_x):
        ...
        stdscr.addstr(
            0,
            0,
            f"📻  Chromecast Terminal Player - {self.player.cast_info.friendly_name}",
            curses.A_BOLD,
        )
        ...
        track_line = (
            "No track playing"
            if path is None
            else f"{path.relative_to(self.music_root)}"
        )
...
class SelectCastDeviceUI:
    ...
    def _try_select(self) -> bool:
        with self.cc_listener._lock:
            if self.cc_listener.services == None or self.selected_index >= len(
                self.cc_listener.services
            ):
                return False
    ...
    def _draw(self, stdscr, max_y, max_x):
        ...
        # ----- Header -------------------------------------------------
        stdscr.addstr(
            0, 0, f"📻  Chromecast Terminal Player - Discovering devices", curses.A_BOLD
        )
            ...
            if self.cc_listener.services != None:
                ...
                for service in self.cc_listener.services:
                    ...
                    stdscr.addstr(
                        index + 2,
                        0,
                        f"[{selected_marker}] '{service.friendly_name}' ({service.model_name}) @ {service.host}:{service.port}",
                    )
                    index += 1
...
def main():
    ...
    print(
        f"✅ Found Chromecast: {cast_info.friendly_name} ({cast_info.host}:{cast_info.port})"
    )
    ...
    with CastPlayer(
        cast, cast_info, music_files, music_root, local_ip, args.port
    ) as player:
        logger.info("▶️  Starting UI (press 'q' to quit)...")
        try:
            curses.wrapper(
                lambda stdscr: ChromecastPlayerUI(
                    player, len(music_files), music_root
                ).run(stdscr)
            )
